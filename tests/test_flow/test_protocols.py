...
def test_picklable_value(builder, manager, value):
...
def test_picklable_with_parens(builder, manager):
    counter = manager.ResettingCounter()
    @builder
    @bn.protocol.picklable()
    @count_calls(counter)
...
@pytest.mark.parametrize("value", PICKLABLE_VALUES)
def test_picklable_value_is_also_dillable(builder, manager, value):
    counter = manager.ResettingCounter()
    @builder
...
def test_dillable(builder, manager):
...
def test_simple_dataframe(builder, manager):
...
def test_simple_dask_dataframe(builder, manager):
    df_value = df_from_csv_str(
        """
    color,number
    red,1
...
def test_multiple_partitions_dask_dataframe(builder, manager):
    df_value = df_from_csv_str(
        """
    color,number
    red,1
...
def test_path_protocol(builder, manager, tmp_path, operation):
    ...
    phrase_file_path_counter = manager.ResettingCounter()
    @builder
    ...
    colors_dir_path_counter = manager.ResettingCounter()
    @builder
    @bn.protocol.path(operation=operation)
